% clc;  clear;      %  THM1_static.m      (Case 1)
% F-404 aircraft engine model borrowed from [(2024) Dr.Gu Yang]
...
lambda1=0.9;    theta2=1.2;   % theta >1,   lambda > 1/theta, 
a1=1.2;    % The parameter of Finsler's lemma
rho=0.6;   % g(t)'*g(t) <= rho x(t)'*x(t),  
c1=0.3;    % The initial energy, x(0)Rx(0) <= c1,   % R=0.1*eye(n_x); 
E_kd=0.0;  E_kc=0.0;     % E_kd=kappa_d,    E_kc=kappa_c,    
T=6;   % <-- settled time, parameter of FTC,  T=30 is simulation time,
sigma1=0.01;   % Threshold parameter,  % \tilde{D}= sigma I,
phi1=0.1;     % eta(0)= phi*x(0)'*x(0),
x(:,1) = [ -1;  0.5;  0.8];   % initial state x(0), 
y_l(1)= phi1*x(:,1)'*x(:,1);   % y_l(1)= \eta(0),    \eta(t)=y_l(n),   
E0 = [ 1 0 0 ; 0 1 0 ; 0 0 1 ];    % R = 0.1*eye(n_x);
...
lmiterm([3 1 1 Omega],(1+1/theta2)*sigma1*C1',C1);    % \tilde{D}= sigma I; 
lmiterm([3 1 2 W1],a1*(1-E_kd)*(1-E_kc)*B1,1);
lmiterm([3 1 2 Omega],(1+1/theta2)*sigma1*C1',1);   
...
lmiterm([3 2 2 Omega],-(1+1/theta2)*(1-sigma1),1);
...
lmiterm([8 1 1 Omega],(1+1/theta2)*sigma1*C2',C2);    % \tilde{D}= sigma I;
lmiterm([8 1 2 W2],a1*(1-E_kd)*(1-E_kc)*B2,1);
lmiterm([8 1 2 Omega],(1+1/theta2)*sigma1*C2',1);     
...
lmiterm([8 2 2 Omega],-(1+1/theta2)*(1-sigma1),1);
...
lmiterm([12 1 1 Omega],(1+1/theta2)*sigma1*C3',C3);    % \tilde{D}= sigma I; 
lmiterm([12 1 2 W3],a1*(1-E_kd)*(1-E_kc)*B3,1);
lmiterm([12 1 2 Omega],(1+1/theta2)*sigma1*C3',1);     
...
lmiterm([12 2 2 Omega],-(1+1/theta2)*(1-sigma1),1);  
...
lmiterm([17 1 1 Omega],(1+1/theta2)*sigma1*C4',C4);    % \tilde{D}= sigma I; 
lmiterm([17 1 2 W4],a1*(1-E_kd)*(1-E_kc)*B4,1);
lmiterm([17 1 2 Omega],(1+1/theta2)*sigma1*C4',1);     
...
lmiterm([17 2 2 Omega],-(1+1/theta2)*(1-sigma1),1);  
