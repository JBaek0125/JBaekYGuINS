...
while n<=(tm/h)
    if n==1
       %% Initialization
        ...
        xd(:,1)= [ -1;  0.5;  0.8];   % initial state x(0)=x(:,1), 
        d_1d(:,1)= 0.05 * sin(1*h);   % disturbance, 
        yd(:,1) = C * xd(:,1);     % y(0)=C*x(0),
        y_ed(:,1)= 0 * yd(:,1);    % y_e(:,n) = y(t_k),  
        y_ld(1) = phid*xd(:,1)'*xd(:,1);     % eta(0) = phid*x(0)'*x(0), 
        ud(:,1) = K * y_ed(:,1);
%         kd=rand(1,tm/h+1);
%         kc=rand(1,tm/h+1);
       ...
       %% Plant
        xd(:,n) = h*( A * xd(:,n-1) + B * ud(:,n-1) + D * d_1d(:,n-1)) + xd(:,n-1);
        yd(:,n) = C * xd(:,n);
        
       %% ET  
        eyd = y_ed(:,n-1) - yd(:,n);   % the triggering error
        y_ld(n)= h * ( -lambda* y_ld(n-1) +  sigmad * yd(:,n)' * yd(:,n) - eyd' * eyd ) + y_ld(n-1);
        
       if eyd' * eyd >= sigmad * yd(:,n)' * yd(:,n) + y_ld(n)
        y_ed(:,n)    = yd(:,n);
        Count4       = Count4 + 1;
        intervals4(n)= (n - r1) * h;
        r1          = n;
       ...
       y_a(:,n)=((kd(n)<=E_kd)*0*y_ed(:,n)) + (  (1-(kd(n)<=E_kd))*( (kc(n)<=E_kc)*g(:,n) + (1-(kc(n)<=E_kc) )*y_ed(:,n)) );
