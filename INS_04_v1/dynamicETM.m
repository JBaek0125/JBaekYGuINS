...
sigmad=0.01;   % Threshold parameter,  % \tilde{D}= sigma I,
phid=0.1;     % eta(0)= phi*x(0)'*x(0),
x(:,1) = [ -1;  0.5;  0.8];   % initial state x(0), 
y_l(1)= phid*x(:,1)'*x(:,1);   % y_l(1)= \eta(0),    \eta(t)=y_l(n),   
...
lmiterm([3 1 1 Omega],(1+1/theta)*sigmad*C1',C1);    % \tilde{D}= sigma I; 
lmiterm([3 1 2 W1],a1*(1-E_kd)*(1-E_kc)*B1,1);
lmiterm([3 1 2 Omega],(1+1/theta)*sigmad*C1',1);   
...
lmiterm([3 2 2 Omega],-(1+1/theta)*(1-sigmad),1);
...
lmiterm([8 1 1 Omega],(1+1/theta)*sigmad*C2',C2);    % \tilde{D}= sigma I;
lmiterm([8 1 2 W2],a1*(1-E_kd)*(1-E_kc)*B2,1);
lmiterm([8 1 2 Omega],(1+1/theta)*sigmad*C2',1);     
...
lmiterm([8 2 2 Omega],-(1+1/theta)*(1-sigmad),1);
...
lmiterm([12 1 1 Omega],(1+1/theta)*sigmad*C3',C3);    % \tilde{D}= sigma I; 
lmiterm([12 1 2 W3],a1*(1-E_kd)*(1-E_kc)*B3,1);
lmiterm([12 1 2 Omega],(1+1/theta)*sigmad*C3',1);     
...
lmiterm([12 2 2 Omega],-(1+1/theta)*(1-sigmad),1);  
...
lmiterm([17 1 1 Omega],(1+1/theta)*sigmad*C4',C4);    % \tilde{D}= sigma I; 
lmiterm([17 1 2 W4],a1*(1-E_kd)*(1-E_kc)*B4,1);
lmiterm([17 1 2 Omega],(1+1/theta)*sigmad*C4',1);     
...
lmiterm([17 2 2 Omega],-(1+1/theta)*(1-sigmad),1);  
