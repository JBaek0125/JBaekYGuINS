...
while n<=(tm/h)
    if n==1
       %% Initialization
        ...
        y_l2(1) = phi1*x2(:,1)'*x2(:,1);     % eta(0) = phi1*x(0)'*x(0), 
        u2(:,1) = K * y_e2(:,1);
%          kd=rand(1,tm/h+1);
%          kc=rand(1,tm/h+1);
       ...
       %% ET  
        ...
        y_l2(n)= h * ( -lambda1* y_l2(n-1) +  sigma1 * y2(:,n)' * y2(:,n) - ey2' * ey2 ) + y_l2(n-1);
        
       if ey2' * ey2 >= 0*sigma1 * y2(:,n)' * y2(:,n) + y_l2(n)
        y_e2(:,n)    = y2(:,n);
        Count2       = Count2 + 1;
        intervals2(n)= (n - r1) * h;
        r1          = n;
       ...
       y_a(:,n)=((kd(n)<=E_kd)*0*y_e2(:,n)) + (  (1-(kd(n)<=E_kd))*( (kc(n)<=E_kc)*g(:,n) + (1-(kc(n)<=E_kc) )*y_e2(:,n)) );
